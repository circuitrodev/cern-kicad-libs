...
These are the KiCad component libraries used at CERN for electronics design.
They have been open-sourced so they can be used freely by designers outside CERN
including, but not only, CERN contractors.

The libraries are the result of automatically converting the work of the fine
folks at CERN's Electronics Design Office, using kicad-cli every night. They
contain symbols and footprints for each component. Please note that 3D models
and datasheets are not included, as many of them are not CERN's IP (for example,
obtained from component vendors).
...
no express or implied warranties of any kind. In addition, we are unfortunately
unable to provide user support, including answering general questions or
assisting with the correct use of the libraries. The “as is” provision therefore
...
conversion process. Feel free to report it [here](insert-link-to-issues) and we
will do our best to follow up and fix it.
