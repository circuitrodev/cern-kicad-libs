...
// The libraries are synced from upstream daily, but the SVG previews are
// regenerated by hand, so the two can be of different vintages. Show both.
async function renderFreshness(idx) {
  const el = $("#freshness");
  const previews = (idx.generated || "").slice(0, 10);
  let libraries = previews;
  try {
    const state = await fetch("mirror-state.json").then(r => r.ok ? r.json() : null);
    if (state && state.upstream_committed) libraries = state.upstream_committed.slice(0, 10);
  } catch { /* the freshness line is optional; the viewer works without it */ }
  if (!previews) return;
  const stale = libraries !== previews;
  el.innerHTML = `libraries ${escapeHtml(libraries)} &middot; previews ${escapeHtml(previews)}` +
    (stale ? ` <span class="stale" title="The preview images were generated before the latest library sync">previews behind</span>` : "");
}
